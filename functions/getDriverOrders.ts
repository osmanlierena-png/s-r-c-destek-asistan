...
Deno.serve(async (req) => {
    try {
        ...
        console.log('🚀 DEPLOY v105 - FRESH BUILD');
        ...
        // Build the function URL for POST using origin from request
...
` : '<div style="background: #dcfce7; border-radius: 8px; padding: 24px; text-align: center; margin-top: 20px;"><p style="color: #166534; font-weight: 600; margin: 0;">✅ You have already responded to these orders.</p></div>'}
</div>
<script>
...
document.getElementById('btn-confirm') && document.getElementById('btn-confirm').addEventListener('click', function() {
    ...
    .then(function(data) {
        if (data.success) {
            ...
            if (data.approvedOrderNumbers && data.approvedOrderNumbers.length > 0) message += 'Approved: ' + data.approvedOrderNumbers.join(', ');
            if (data.rejectedOrderNumbers && data.rejectedOrderNumbers.length > 0) message += ' | Rejected: ' + data.rejectedOrderNumbers.join(', ');
